//
//  CustomCombineDataSourceVC.swift
//  UIComponentTutorial
//
...
class CustomCombineDataSource: NSObject, UITableViewDataSource {
    ...
    func pushDataList(_ updatedDataList: [DummyData], tableView: UITableView) {
...
// MARK: - Extension
/// Sink 정의를 보면 매개변수로 (Self.Output) -> Void) 형태의 클로저를 받는다. 이 형태를 만족하는 로직 함수를 만들자.
...
extension UITableView {
    ...
    func customItems() -> ([DummyData]) -> Void {
        ...
        return { (updatedDataList: [DummyData]) in
            dataSource.pushDataList(updatedDataList, tableView: self) // 리로드
        }
    }
}

final class CustomCombineDataSourceVC: UIViewController {
    
    // Combine 메모리 처리를 위해 생성
    var subscriptions = Set<AnyCancellable>()
    
    // Published 하게 되면 dummies 데이터가 추가나 값 변경시 이벤트를 받을 수 있다.
