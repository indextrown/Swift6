...
final class MainVC: UIViewController {
    ...
    // var todosVM: TodosVMClosure = TodosVMClosure() // 일단은 생성될때 바로 넣어주도록 처리
...
//
        ...
        self.todosVM
            ...
            .subscribe { [weak self] updatedTodos in
                guard let self = self else { return }
                self.todos = updatedTodos
                self.myTableView.reloadData()
            }
        ...
        self.todosVM
            ...
            .withUnretained(self)
            .observe(on: MainScheduler.instance)
            .subscribe { mainVC, updatedTodos in
        ...
        // MARK: - 페이지 변경
        self.todosVM.notifyCurrentPageChanged = { [weak self] curentPage in
            guard let self = self else { return }
            DispatchQueue.main.async {
                self.pageInfoLabel.text = "페이지: \(curentPage)"
                self.myTableView.reloadData()
            }
        }
        ...
        // MARK: - 다음페이지 존재 여부
        self.todosVM.notifyNextPage = { [weak self] hasNext in
            guard let self = self else { return }
