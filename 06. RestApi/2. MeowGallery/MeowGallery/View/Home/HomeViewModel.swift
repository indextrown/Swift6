    ...
    func fetchCats(page: Int) {
        CatApiService.fetchCats(page: page)
            .sink { completion in
                ...
                if page == 1 {
                    self.catList = convertedCats
                } else {
                    self.catList.append(contentsOf: convertedCats)
                }
...
// MARK: - viewLogic
extension HomeViewModel {
    func infinityScroll (cat: AppCat) {
        guard let last = catList.last else { return }
        if last.id == cat.id {
            loadMore()
        }
    }
