...
//    }
    ...
    func fetchMore() {
        ...
        if searchTerm.value.count > 0 { // 검색어가 있으면
            self.searchTodos(searchTerm: searchTerm.value, page: currentPage.value + 1)
        } else {
    ...
    init() {
        ...
        currentPageInfo = self.currentPage
            .map { "페이지: \($0)" }
        
        // MARK: - PageInfo가 변경되면 그에 따라서 구독처리가 되었기 떄문에 currentPage라는 behaviorrelay도 바뀜
        pageInfo
        ...
        searchTerm
            ...
            .subscribe(onNext: { vm, searchTerm in
                print(#fileID, #function, #line, "- input: \(searchTerm)")
                
                if self.searchTerm.value.count > 0 {
...
extension TodosVMRx {
    ...
    func fetchTodos(page: Int = 1) {
        ...
        Observable.just([])
            ...
            // 옵저버블의 이벤트에 따라 액션 처리 가능 - 데이터 변경이 아닌 로그를 찍거나, 상태를 바꾸거나, 에러를 처리할 때
            .do(onError: { error in
            ...
            // subscribe는 옵저버블을 구독해서 최종적으로 결과를 사용하는 부분
            // 네트워크 요청 결과를 UI에 반영하거나, 저장하는 식으로 결과를 실질적으로 소비하는 마지막 단계
