...
// genericAsyncToPublisher를 조금 더 간단하게
extension Publisher {
...
extension TodosAPI {
    static func fetchTodosAsyncToObservable(page: Int) -> Observable<BaseListResponse<Todo>> {
        ...
        // 값을 내보낸다
        return Observable.create { (observer: AnyObserver<BaseListResponse>) in
            //
            Task {
                do {
                    let asyncResult = try await fetchTodosWithAsync(page: page)
...
// MARK: - 팁: async 행하는 것을 클로저로 빼서 밖으로 매개변수로 뺴준다고 생각하자. 어떤 async함수든지 호출만 되면 됨
extension TodosAPI {
    func genericAsyncToObservable<T>(asyncWork: @escaping () async throws -> T) -> Observable<T> {
        // 값을 내보낸다
...
// genericAsyncToObservable를 조금 더 간단하게
        ...
        let retryCount: Int = 3
            ...
            Task {
                for _ in 0..<retryCount {
                    do {
                        let result = try await TodosAPI.fetchTodosWithAsync(page: 11111)
                        print("result: \(result)")
                    } catch {
                        print("eerror: \(error)")
                        
                        // 조건 - noContentError가 아닐경우는 else 실행 = 종료
                        guard case TodosAPI.ApiError.noContentError = error else { throw error }
...
extension Task where Failure == Error  {
    ...
    // MARK: - Static은 메모리를 생성하지 않아도 즉 인스턴스를 만들지 않아도 접근할 수 있다
        ...
        return Task {
            for _ in 0..<retryCount {
                do {
                    ...
                    // 조건
                    // when에 대한 결과가 true가 아니라면 에러를 던져서 반복 종료
