...
final class TodosVM: ObservableObject {  // ObservableObject -> 변경에 대한 감지
    ...
    init() {
        ...
        TodosAPI.fetchTodosWithObservableData(page: 111)
        //.retry() // 에러가 타지 않을떄까지 무한 반복
            .retry(3) // 3번까지 반복
            .subscribe {
                print("onNext: \($0)")
            } onError: {
...
extension TodosVM {
    ...
    // MARK: - 기본적인 방식
    func fetchTodosWithObservableDataRetry() {
        var requestCount: Int = 0
        let retryCount: Int = 3
        TodosAPI.fetchTodosWithObservableData(page: 1111111111)
        //.retry() // 에러가 타지 않을떄까지 무한 반복
        // .retry(3) // 3번까지 반복
            .retry(when: { (observableError: Observable<TodosAPI.ApiError>) in //
                ...
                .flatMap { error in
                    ...
                    //                    if let apiError = error as? TodosAPI.ApiError {
                    //                        switch apiError {
                    //                        case .noContentError:
                    //                            return Observable<Void>.just(())
                    //                                .delay(.seconds(3), scheduler: MainScheduler.instance)
    ...
    // MARK: - 제네릭 방식 - retryWithDelayAndConditionCatch(), retryWithDelayAndCondition()
    func retryWithDelayAndConditionCatch() {
        TodosAPI.fetchTodosWithObservableData(page: 1111111111)
        //            .retryWithDelayAndCondition(retryCount: 3, delay: 2, when: { error in
        //                print("error0809: \(error)")
        //                if case TodosAPI.ApiError.unAuthorized = error {
        //                    return true
        //                }
        //                return false
        //            })
        // .retryWithDelayAndCondition(retryCount: 3, delay: 2)
            .retryWithDelayAndConditionCatch(retryCount: 3, delay: 2, when: { error in
                print("에러디버그: \(error)")
                if case TodosAPI.ApiError.noContentError = error {
                    return true
                }
...
extension TodosVM {
    func combineRetry1() {
        TodosAPI.fetchTodosWithPublisher(page: 1111111)
            .print("Combine 리트라이")
    ...
    func combineRetry2(retryCount: Int = 1) {
        ...
        TodosAPI.fetchTodosWithPublisher(page: 1111111)
            .tryCatch({ error in
                ...
                return Just(Void())
                    .delay(for: 3, scheduler: DispatchQueue.main)
                    .flatMap { _ in
                        return TodosAPI.fetchTodosWithPublisher(page: 1111111)
                    }
                    .retry(retryCount)
                    .eraseToAnyPublisher()
            })
    ...
    func combineRetry3() {
        TodosAPI.fetchTodosWithPublisher(page: 1)
            .retryWithDelayAndCondition(retryCount: 3, delay: 2) { error in
                print("한번더")
//                if case TodosAPI.ApiError.noContentError = error {
//                    return true
//                }
                if case TodosAPI.ApiError.decodingError = error {
                    return true
                }
                return false
            }
...
extension TodosVM {
    func asyncRetry() {
        let fetchTodosTask = Task.retry(retryCount: 3, delay: 2) { error in
            if case TodosAPI.ApiError.noContentError = error {
                return true
            }
            return false
